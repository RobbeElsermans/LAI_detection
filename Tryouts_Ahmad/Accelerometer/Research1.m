...
for folderIdx = 1:length(folders)
    ...
    for fileIdx = 1:length(bagFiles)
        ...
        % Calculate the mean of xAccelerations
        meanXAccelerations(fileIdx) = mean(xAccelerations);
    ...
    results{folderIdx, 2} = [meanXAccelerations, motionStatus];
end

% Compare overall results
% You can perform additional analysis or comparison here based on 'results'

% Compare overall results
% Calculate average mean X-Acceleration for each folder
meanXAccFolders = zeros(length(folders), 1);
for folderIdx = 1:length(folders)
    meanXAccs = results{folderIdx, 2}(:, 1);
    meanXAccFolders(folderIdx) = mean(meanXAccs);
end

% Display average mean X-Accelerations for each folder
fprintf("Average Mean X-Acceleration:\n");
for folderIdx = 1:length(folders)
    fprintf("%s: %.4f\n", results{folderIdx, 1}, meanXAccFolders(folderIdx));
end
